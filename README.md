...
No Render ao fazer o deploy precisamos ajustar o Start Command para `node dist/server.js` e o Build Command para `npm install; npm run build; npx prisma db push`. Também precisamos adicionar uma variável de ambiente `DATABASE_URL` com a URL de conexão com o banco de dados. O JWT Secret pode ser adicionado da mesma forma.
